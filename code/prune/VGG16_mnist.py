...
class ModifiedVGG16Model(torch.nn.Module):
    ...
    def __init__(self):
        ...
        new_conv = None
        layers = []
        for i, (name,module) in enumerate(list(features._modules.items())):
            if i == 0:
        ...
        for param in self.features.parameters():
            ...
            param.requires_grad = True #Yangcy modified 2018/12/21 because hook can only be registed when grad is required
...
class FilterPrunner:
    ...
    def forward(self, x):
        ...
        activation_index = 0
        for layer, (name, module) in enumerate(list(self.model.features._modules.items())):
            #print(x)
            x = module(x)
...
class PrunningFineTuner_VGG16:
    ...
    def __init__(self, train_path, test_path, model, use_gpu, torch_version = 0.4):
        ...
        self.prunner = FilterPrunner(self.model,use_gpu)
    ...
    def train_batch(self, optimizer, batch, label, rank_filters):
        ...
        if(not self.use_gpu):
            self.prunner.model = self.prunner.model.cpu()
        ...
        if rank_filters:
            ...
            self.criterion(output, label).backward() #self.criterion = torch.nn.CrossEntropyLoss() line 300
        else:
            self.criterion(self.model(input), label).backward()
            optimizer.step()

    def train_epoch(self, optimizer=None, rank_filters=False, number = -1):
        for batch, label in self.train_data_loader:
            if(number!=-1):
                batch = batch[label==number]
    ...
    def get_candidates_to_prune(self, num_filters_to_prune, number = -1):
        self.prunner.reset()
        ...
        self.train_epoch(rank_filters=True,number = number)
    ...
    def adjstFtInd(self,prunned, target):
        res = []
        for lyrind, fltind in target:
            former = []
            for prnd in prunned:
                for pndlyr, pndflt in prnd:
                    if(pndlyr==lyrind):
                        former.append(pndflt)
            former.sort()
            for i in former:
                if (i>fltind):
                     break
                fltind+=1
            res.append((lyrind,fltind))
        return res

    def prune(self, num = -1):
        ...
        if True:
            ...
            for it_k in range(iterations):
                ...
                prune_targets = self.get_candidates_to_prune(num_filters_to_prune_per_iteration,number = num)
                layers_prunned = {}
                ...
                prunnedFilters.append(prune_targets)
                for layer_index, filter_index in prune_targets:
                    model = prune_vgg16_conv_layer(model, layer_index, filter_index)#COMM: THE LINE TO PRUNE defined in prune.py
            ...
            # print("Fine tuning to recover from prunning iteration.")
                ...
                last_acc = self.train(optimizer, epoches=3)
                torch.save(model,"./pruned/vgg16_mnist_%d_prunned_%d" %( num ,it_k))
            ...
            torch.save(model, "vgg16_mnist_%d_prunned" % num)
            with open("prunned_%d.pkl" %num, "wb")as f:
                pkl.dump(prunnedFilters,f)
